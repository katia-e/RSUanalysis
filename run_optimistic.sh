...
nTerms=2; echo $nTerms terms will be calculated

# stopCrit defines stopping criteria
stopCrit=0.1 #0.000001; echo Stopping criteria is $stopCrit #0.000001

# w defines parameter of weighted jacoby
w=0.95; echo method: weighted Jacoby with parameter $w

### Simulaiton parameters
lamMax=50; echo Range of system loads [0...$lamMax]
nSimSamples=35; echo nSimSamples = $nSimSamples defines number of samples
nSimLoops=300000000; echo nSimLoops = $nSimLoops defines number of simulation loops to obtain one sample
...
# Run simulation (directly with matlab)
...
# run simulation with compiled matlab file (compile in /Simulation folder with mcc -m -R -nojvm -R -nodisplay -R -singleCompThread run_simulation.m -o run_simulation)
